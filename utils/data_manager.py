...
class DataManager(object):
    def __init__(self, dataset_name, shuffle, seed, init_cls, increment, train_trans=None, test_trans=None):
        ...
        self._setup_data(dataset_name, shuffle, seed, train_trans, test_trans)
    ...
    def get_dataset_with_split(self, indices, source, mode, appendent=None, val_samples_per_class=0):
        ...
        if self.new_trans:
            ...
            if mode == "train":
                trsf = transforms.Compose(
                    [*self._train_trsf, *self._common_trsf])
            elif mode == "test":
                trsf = transforms.Compose(
                    [*self._test_trsf, *self._common_trsf])
            else:
                raise ValueError("Unknown mode {}.".format(mode))
        ...
        train_data, train_targets = np.concatenate(
            train_data), np.concatenate(train_targets)
    ...
    def _setup_data(self, dataset_name, shuffle, seed, train_trans, test_trans):
        ...
        # Transforms
        if self.new_trans:
            self._train_trsf = train_trans
            self._test_trsf = test_trans
        else:
